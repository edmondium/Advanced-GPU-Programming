...
__global__ void maxKernel(int * array) {
	...
	cg::thread_block_tile<16> tiledPartition = cg::tiled_partition<16>(thisThreadBlock);
	...
	int maxValue = maxFunction(tiledPartition, shmem_temp + shmemOffset, myValue);
	thisThreadBlock.sync();

	// NOTE: We now have local max values for each tile; we still need to find the max value amongst them
	// TODO: Open `if` not on the index `threadIndex` of the global group being 0 but on the index of the local partition being 0
