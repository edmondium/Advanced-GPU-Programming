...
__device__ int maxFunction(cg::thread_block_tile<tile_size> g, int value) {
	...
	for (int i = g.size() / 2; i > 0; i /= 2) {
		...
		value = max(value, g.shfl_down(value, i));
	}
	return value;
}
...
__global__ void maxKernel(int * array) {
	...
	int maxValue = maxFunction<tile_size>(tiledPartition, myValue);
...
int main() {
	...
	const int N = 128;
	int * array;
	checkCudaErrors(cudaMallocManaged(&array, sizeof(int) * N));
